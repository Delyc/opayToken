...
bool IsCountValid(int Count, uint16_t MaxCount, int Value,
                  uint16_t UsedCounts) {
  if (Value == COUNTER_SYNC_VALUE) {
    // We don't need to check if it's below the max as those tokens aren't tried
    // anyway
...
void MarkCountAsUsed(int Count, uint16_t* MaxCount, uint16_t* UsedCounts,
  ...
  if (Count % 2 || Value == COUNTER_SYNC_VALUE || Value == PAYG_DISABLE_VALUE) {
    // If it was a SET_CREDIT token or it was Counter Sync then we mark all past
    // tokens as used
    ...
    if (Count > *MaxCount) {
      ...
      // We move all of the tokens marked as used to their new position relative
      // to the new max count
      for (int i = RelativeCount + 1;
           i < MAX_UNUSED_OLDER_TOKENS - RelativeCount; i++) {
        NewUsedCount =
            SET_BIT(NewUsedCount, i, CHECK_BIT(*UsedCounts, i - RelativeCount));
      }
      *UsedCounts = NewUsedCount;
    } else {
      // If the token is older than the max one, we just mark this one as used
      // but don't shift any other tokens
