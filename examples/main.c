  ...
  while (NoToken) {
    ...
    if (LastKey == STAR_KEY) {
      if (TokenEntryAllowed()) {
        ...
        BlinkRedLED(1);
#ifdef DEBUG
        printf("\nToken entry locked for %d seconds",
...
int main(int argc, const char *argv[]) {
  ...
  printf(
      "We're waiting for the * character to start recording the key presses. "
      "\n(You need to press ENTER after the key presses for the simulator to "
  ...
  while (1) {
    ...
    Output = GetDataFromToken(InputToken, &TokenCount, &UsedTokens,
